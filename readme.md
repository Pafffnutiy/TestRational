...
с помощью сеттера знаменателя (`setDenominator(int)`).
    ...
    2. Фактический результа: знаменатель рационального числа становится нулевым
2. Тест №10 в данном тесте проверяется работоспособность метода `less()`. Тест не проходится в случае 
различных числителей и знаменателей у двух дробей. Это связано с ошибкой реализации метода. В строке 
кода
```java
return this.numerator * rational.getDenominator() < getNumerator() * this.denominator;
```
метод `getNumerator()` вызывается для неявного параметра `this`. Так как в ТЗ нет отдельного уточнения, 
что этот метод должен работать именно так, то считаем, что сравнение рациональных чисел должно 
производиться согласно правилам математики. По данным правилам метод getNumerator должен вызываться для 
объекта rational.
3. Тест №11 в данном тесте проверяется работоспособность метода lessOrEqual(). Тест не проходит, так 
как проверяемый метод использует в реализации неверно реализованный метод less (см. пункт 2 примечания).
